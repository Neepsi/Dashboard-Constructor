...
function Dashboardd(route) {       
    ...
    const [delaisData, setDelaiData] = useState();
    
    const fetchData = () =>
    {       
            Axios.get("http://localhost:3001/figuresDel",{route: route}).then((response)=>{
               (response['data'].length === 0 || response['data'][0]['delais']=== null) ? setInputList([]) : setInputList(JSON.parse(response['data'][0]['delais']).figures_delais);                               
               //console.log(JSON.parse(response['data'][0]['delais']).figures_generalites)
            })
